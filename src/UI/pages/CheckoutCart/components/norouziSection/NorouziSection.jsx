    ...
    <>
      <div className="w-100 d-flex flex-column">
        {factor?.cartItems?.length > 0 &&
          factor?.cartItems?.filter((item) => item.cartItemType == 5)?.length >
            0 && (
            <>
              {/* <hr className="my-3" /> */}
              {/* <SectionCalculation
              ...
              /> */}
              {factor?.cartItems
                ?.filter((item) => item.cartItemType == 5)
                ?.map((item) =>
                  norouziProducts_methods.doesExistsInBasket(
                    item?.product?.id
                  ) ? (
                    <DessertItem2
                      className="shadow"
                      ...
                      deleteDessertHandler={() => {
                        norouziProducts_methods.deleteItem(item?.product?.id);
                      }}
                      dessertMinusButtonHandler={() => {
                        norouziProducts_methods.decrement(item?.product?.id);
                      }}
                      dessertPlusButtonHandler={() => {
                        norouziProducts_methods.increment(item?.product?.id);
                      }}
                    />
                  ) : (
                    <></>
