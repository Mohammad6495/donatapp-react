...
import CreamyCookieDetailsItem from "./components/CreamyCookieDetailsItem";
...
const CreamyCookieDetails = () => {
  ...
  const handleNavigateToCheckOutCart = () => {
    ...
    // } else
    // {
    if (isAvailability.isAvailableCreamyCookie) {
      cookieProducts_methods.createAndAddItem(boxRows);
      setShouldGoToBasket(true);
    } else {
      toast.warn(isAvailability.creamyCookieDescription);
    }
    // }
  };
  ...
  const onBoxRowDelete = (id) => {
    ...
    handleDeleteCreamyCookieItem(id);
  };
  useEffect(() => {
    if (boxRows?.length == 0) {
      navigate(-1);
  ...
  return (
    ...
    >
      ...
      <div className="w-100">
        {/*  */}
        <div className="caro-bg p-2 rounded-2">
          <span className="text-white fs-7">
            تمامی شیرینی ها کیلویی {formatPrice(creamyCookiePrice)} تومان است
          </span>
        </div>
        {/* boxes */}
        <div className="d-flex jusify-content-between align-items-stretch w-100 mx-1 mb-2 mt-3">
...
export default CreamyCookieDetails;
